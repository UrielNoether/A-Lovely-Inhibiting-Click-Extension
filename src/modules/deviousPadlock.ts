...
function createDeviousPadlock(): void {
	...
	// @ts-ignore
	AssetGet("Female3DCG", "ItemMisc", deviousPadlock.Name).Description = "Permanent Padlock";//changed from "Devious Padlock"
...
export function hasKeyToPadlock(groupName: AssetGroupItemName, target1: Character, target2: Character): boolean {
	...
	if (target1.MemberNumber === 169853 && target2.MemberNumber !== 169853) return true; //true if it's Melinda, but not Melinda on herself 
	return false;
}
	...
	hookFunction("DialogSetStatus", HookPriority.ADD_BEHAVIOR, (args, next) => {
		...
		if (
			...
			if (CurrentCharacter.IsPlayer()) {
				args[0] = "This item is locked by a permanent padlock, you are stuck with it.";
			} else {
				args[0] = `"This item is locked by a permanent padlock, ${getNickname(CurrentCharacter)} is stuck with it.`;
			} 
		}
		next(args);
	});
				...
				return [
					{
						...
						tooltipText: `Locked with Permanent Padlock from ALICE mod`
					}
				];
			}
		}
